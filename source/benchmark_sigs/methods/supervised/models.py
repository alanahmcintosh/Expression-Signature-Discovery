...
def fit_alt_to_expr_weights_svm(
    ...
    n_bootstraps=20,
    sample_fraction=0.5,
    threshold_rule="mean+sd",
    threshold_z=1.0,
):
    """
    Stability selection for LinearSVR.
    For each resample and each gene:
      1. fit LinearSVR
      2. threshold abs(coef) within that gene
      3. accumulate selection counts
    Returns predictors x genes matrix of selection frequencies in [0,1].
    """
    X, Y = align_XY(X_alt, Y_expr)
    rng = np.random.default_rng(random_state)
    ...
    for b in range(n_bootstraps):
        ...
        for j, gene in enumerate(Y.columns):
            ...
            svr.fit(Xb.values, yb)

            scores = np.abs(svr.coef_)
            mask = score_threshold_mask(
                scores,
                rule=threshold_rule,
                z=threshold_z,
